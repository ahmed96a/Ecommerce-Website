...
<!--Start The Page-->
    ...
    <div class="container">
        <h1 class="text-center">Show Items By Tag <?php echo $_GET['name']; ?></h1>
        <div class="row">
        <?php
            $items = $db->getRows("select * from items where tags like ? and approve = 1 order by id desc", array("%{$_GET['name']}%"));
            if(!empty($items))
            {
